...
def CALCULATE_BETTY_DEMO_SCORE_V1(trainData: pd.DataFrame, row: pd.Series) -> tuple:
    ...
    return posterior, len(priority) / n
...
def CALCULATE_BETTY_DEMO_SCORE_V2(trainData: pd.DataFrame, row: dict) -> tuple:
    """
...
def GENERATE_BETTY_DEMOGRAPHIC_SCORE(
    ...
    Note:
    In this function, `CALCULATE_BETTY_DEMO_SCORE_V1` is called, which is significantly faster than its V2 counterpart.
    ...
    test["DEMOGRAPHIC_FEATURES_USED"] = np.round(featuresUsed, 2)
    return test
