...
def map_columns(file_path, new_name_dict, industry_profile):
    ...
    return new_file_path, df_renamed_appended
     

# Example usage: 
# python3 python_models/column_mapping.py ~/Downloads/'Neighbor List - Victor Bassett - for DirectSkip.csv' '{"First Name": "owner_first_name","Last Name": "owner_last_name","Mailing Zip":"mail_zip_code","Property Zip":"property_zip_code","Custom 1 (APN)":"apn","Custom 2 (Lot Size)":"lot_acreage"}'
# Script starts here
msg = "Column Mapping Script - Converts customer provided file header naming convent to RWR"
...
for i,arg in enumerate(sys.argv):
    ...
    if arg == "--custom_mapping":
        new_name_dict = ast.literal_eval(sys.argv[i+1])
    if arg == "--industry_profile":
        industry_profile = json.loads(sys.argv[i+1])
new_file_path = ""

try:
    new_file_path, df = map_columns(file_path, new_name_dict, industry_profile)
    df.to_csv(new_file_path, index=False)
    logging.info({"success":"True","OriginalFilePath": file_path,"NewFilePath": new_file_path, "mapped_cols": renamed_cols})
except Exception as e:
    exc = str(e).replace('"','')
    exc = str(exc).replace("'",'')
    logging.error({"success":"False","OriginalFilePath": file_path,"NewFilePath": new_file_path, "error": "column_mapping_failure", "error_details": exc, "mapped_cols": renamed_cols})
